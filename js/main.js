...
import { PAGES } from './config.js?v=4.66';
import { createCurator } from './curator.js?v=4.66';
import { createCat } from './cat.js?v=4.66';
import { initHUD } from './hud.js?v=4.66';
import { initTransitions } from './transition.js?v=4.66';
import { initTimeOfDay } from './timeofday.js?v=4.66';
import { initNarrator } from './narrator.js?v=4.66';
import { initAnalytics } from './analytics.js?v=4.66';
import { initPalette } from './palette.js?v=4.66';
import { initResumePreview, resolveResumePdf } from './resume-preview.js?v=4.66';
import { initPrefetch } from './prefetch.js?v=4.66';

const page = document.body.dataset.page;

// privacy-aware visit beacons (no-ops if the Apps Script endpoint is cleared,
...
// service worker — offline shell + instant repeat loads (PWA). Registered after
// load so it never competes with first paint; a failure is a silent no-op.
...
if (document.getElementById('contact-form')){
  const { initContact } = await import('./contact.js?v=4.66');
  initContact();
}
...
// phones also retire the cursor-follow swarm on the HOME hub — with no cursor to
// chase it's just noise; a calm sine wave takes its place (see the alley branch).
const phoneWaveHome = isPhone && page === 'alley';
if (canvas && !(isPhone && PHONE_NO_CURATOR.has(page)) && !phoneWaveHome){
...
addEventListener('nekocatcher:boot', async () => {
  const { bootGame } = await import('./game.js?v=4.66');
  bootGame();
});
...
const { initDynamicMenu } = await import('./dynamic-menu.js?v=4.66');
initDynamicMenu(curator);

if (page === 'alley'){
  const { initAlley } = await import('./alley.js?v=4.66');
  initAlley(curator);
  // phones: the retired swarm becomes a calm sine wave along the lower screen
  if (phoneWaveHome){
    const { initPhoneWave } = await import('./phonewave.js?v=4.66');
  ...
  if (!isPhone){
    const { initFlows } = await import('./flows.js?v=4.66');
    initFlows();
  }
  // meteors: a subtle full-screen comet loop; skip on phones (battery + heat)
  if (!isPhone){
    const { initMeteors } = await import('./meteors.js?v=4.66');
    initMeteors();
  }
  // generative lo-fi lives only in the hub (off by default, one HUD toggle)
  const { initAmbient } = await import('./audio.js?v=4.66');
  initAmbient();
}
...
if (page === 'about'){
  const { initAbout } = await import('./about.js?v=4.66');
  initAbout(curator);
  if (!isPhone){
    const { initMeteors } = await import('./meteors.js?v=4.66');
    initMeteors();
...
if (page === 'work'){
  const { initWork } = await import('./work.js?v=4.66');
  await initWork();
}
if (page === 'projects'){
  const { initProjects } = await import('./projects.js?v=4.66');
  await initProjects();
}
if (page === 'papers'){
  const { initWins } = await import('./wins.js?v=4.66');
  await initWins();
}

if (document.querySelector('.chapter')){
  const { initPanels } = await import('./panels.js?v=4.66');
  initPanels();
}
...
if (document.body.classList.contains('world-paper')){
  const { initCompanion } = await import('./companion.js?v=4.66');
  initCompanion();
  const { initBackTop } = await import('./backtop.js?v=4.66');
  initBackTop();   // phone-only "↑ TOP" for the long chapters (self-gates)
}

if (page === 'stall'){
  const { initStall } = await import('./stall.js?v=4.66');
  initStall();
}

if (page === 'resume' && document.getElementById('converter')){
  const { initConverter } = await import('./converter.js?v=4.66');
  initConverter();
}
...
if (document.querySelector('.chapter .panel')){
  const { initGlyphBorders } = await import('./glyphborder.js?v=4.66');
  initGlyphBorders();
}
...
if (location.hash === '#debug'){
  const { subscribe, perf } = await import('./ticker.js?v=4.66');
