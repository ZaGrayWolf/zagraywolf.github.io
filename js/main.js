...
import { PAGES } from './config.js?v=4.39';
import { createCurator } from './curator.js?v=4.39';
import { createCat } from './cat.js?v=4.39';
import { initHUD } from './hud.js?v=4.39';
import { initTransitions } from './transition.js?v=4.39';
import { initTimeOfDay } from './timeofday.js?v=4.39';
import { initNarrator } from './narrator.js?v=4.39';
import { initAnalytics } from './analytics.js?v=4.39';
import { initPalette } from './palette.js?v=4.39';
import { initResumePreview, resolveResumePdf } from './resume-preview.js?v=4.39';
...
if (document.getElementById('contact-form')){
  const { initContact } = await import('./contact.js?v=4.39');
  initContact();
}
...
addEventListener('nekocatcher:boot', async () => {
  const { bootGame } = await import('./game.js?v=4.39');
  bootGame();
});
...
const { initDynamicMenu } = await import('./dynamic-menu.js?v=4.39');
initDynamicMenu(curator);

if (page === 'alley'){
  const { initAlley } = await import('./alley.js?v=4.39');
  initAlley(curator);
  const { initFlows } = await import('./flows.js?v=4.39');
  initFlows();
  // meteors: a subtle full-screen comet loop; skip on phones (battery + heat)
  if (!isPhone){
    const { initMeteors } = await import('./meteors.js?v=4.39');
    initMeteors();
  }
  // generative lo-fi lives only in the hub (off by default, one HUD toggle)
  const { initAmbient } = await import('./audio.js?v=4.39');
  initAmbient();
}
...
if (page === 'about'){
  const { initAbout } = await import('./about.js?v=4.39');
  initAbout(curator);
  if (!isPhone){
    const { initMeteors } = await import('./meteors.js?v=4.39');
    initMeteors();
...
if (page === 'work'){
  const { initWork } = await import('./work.js?v=4.39');
  await initWork();
}
if (page === 'projects'){
  const { initProjects } = await import('./projects.js?v=4.39');
  await initProjects();
}
if (page === 'papers'){
  const { initWins } = await import('./wins.js?v=4.39');
  await initWins();
}

if (document.querySelector('.chapter')){
  const { initPanels } = await import('./panels.js?v=4.39');
  initPanels();
}
...
if (document.body.classList.contains('world-paper')){
  const { initCompanion } = await import('./companion.js?v=4.39');
  initCompanion();
  const { initBackTop } = await import('./backtop.js?v=4.39');
  initBackTop();   // phone-only "↑ TOP" for the long chapters (self-gates)
}

if (page === 'stall'){
  const { initStall } = await import('./stall.js?v=4.39');
  initStall();
}

if (page === 'resume' && document.getElementById('converter')){
  const { initConverter } = await import('./converter.js?v=4.39');
  initConverter();
}
...
if (document.querySelector('.chapter .panel')){
  const { initGlyphBorders } = await import('./glyphborder.js?v=4.39');
  initGlyphBorders();
}
...
if (location.hash === '#debug'){
  const { subscribe, perf } = await import('./ticker.js?v=4.39');
