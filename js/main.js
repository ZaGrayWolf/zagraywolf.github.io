...
import { PAGES } from './config.js?v=4.58';
import { createCurator } from './curator.js?v=4.58';
import { createCat } from './cat.js?v=4.58';
import { initHUD } from './hud.js?v=4.58';
import { initTransitions } from './transition.js?v=4.58';
import { initTimeOfDay } from './timeofday.js?v=4.58';
import { initNarrator } from './narrator.js?v=4.58';
import { initAnalytics } from './analytics.js?v=4.58';
import { initPalette } from './palette.js?v=4.58';
import { initResumePreview, resolveResumePdf } from './resume-preview.js?v=4.58';
import { initPrefetch } from './prefetch.js?v=4.58';
...
if (document.getElementById('contact-form')){
  const { initContact } = await import('./contact.js?v=4.58');
  initContact();
}
...
addEventListener('nekocatcher:boot', async () => {
  const { bootGame } = await import('./game.js?v=4.58');
  bootGame();
});
...
const { initDynamicMenu } = await import('./dynamic-menu.js?v=4.58');
initDynamicMenu(curator);

if (page === 'alley'){
  const { initAlley } = await import('./alley.js?v=4.58');
  initAlley(curator);
  // phones: the retired swarm becomes a calm sine wave along the lower screen
  if (phoneWaveHome){
    const { initPhoneWave } = await import('./phonewave.js?v=4.58');
  ...
  if (!isPhone){
    const { initFlows } = await import('./flows.js?v=4.58');
    initFlows();
  }
  // meteors: a subtle full-screen comet loop; skip on phones (battery + heat)
  if (!isPhone){
    const { initMeteors } = await import('./meteors.js?v=4.58');
    initMeteors();
  }
  // generative lo-fi lives only in the hub (off by default, one HUD toggle)
  const { initAmbient } = await import('./audio.js?v=4.58');
  initAmbient();
}
...
if (page === 'about'){
  const { initAbout } = await import('./about.js?v=4.58');
  initAbout(curator);
  if (!isPhone){
    const { initMeteors } = await import('./meteors.js?v=4.58');
    initMeteors();
...
if (page === 'work'){
  const { initWork } = await import('./work.js?v=4.58');
  await initWork();
}
if (page === 'projects'){
  const { initProjects } = await import('./projects.js?v=4.58');
  await initProjects();
}
if (page === 'papers'){
  const { initWins } = await import('./wins.js?v=4.58');
  await initWins();
}

if (document.querySelector('.chapter')){
  const { initPanels } = await import('./panels.js?v=4.58');
  initPanels();
}
...
if (document.body.classList.contains('world-paper')){
  const { initCompanion } = await import('./companion.js?v=4.58');
  initCompanion();
  const { initBackTop } = await import('./backtop.js?v=4.58');
  initBackTop();   // phone-only "↑ TOP" for the long chapters (self-gates)
}

if (page === 'stall'){
  const { initStall } = await import('./stall.js?v=4.58');
  initStall();
}

if (page === 'resume' && document.getElementById('converter')){
  const { initConverter } = await import('./converter.js?v=4.58');
  initConverter();
}
...
if (document.querySelector('.chapter .panel')){
  const { initGlyphBorders } = await import('./glyphborder.js?v=4.58');
  initGlyphBorders();
}
...
if (location.hash === '#debug'){
  const { subscribe, perf } = await import('./ticker.js?v=4.58');
